...
zip -r ${NAME}-modules.zip nodejs/node_modules && \
    ...
    aws lambda create-function --function-name ${NAME} --zip-file fileb://${NAME}.zip --handler index.handler --runtime nodejs10.x --role $1 --timeout 30 --layers ${LAYER_VERSION} --environment Variables="{}" && \
